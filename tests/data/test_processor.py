...
from data.processor import DataProcessor # Assuming DataProcessor is in src/data/processor.py
...
class TestFeatureGenerationMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if os.path.exists(DUMMY_CONFIG_PATH): # DUMMY_CONFIG_PATH is 'config/features_config.yaml'
            os.remove(DUMMY_CONFIG_PATH)
    ...
    def test_add_bollinger_bands_uses_config_params(self, mocked_bbands):
        ...
        self.assertTrue(args[0].equals(self.dummy_df['Close'])) # Check the 'close' series
