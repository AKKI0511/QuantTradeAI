...
class DataLoader:
    ...
    def __init__(
        ...
        self.timeframe = data_cfg.timeframe or "1d"  # Default to daily data
    ...
    def _fetch_single(self, symbol: str, refresh: bool) -> Optional[pd.DataFrame]:
        ...
        try:
            if self.use_cache and not refresh and self._is_cache_valid(cache_file):
                ...
                logger.info(f"Fetching data for {symbol} with timeframe {self.timeframe}")
                df = self.data_source.fetch(symbol, self.start_date, self.end_date, self.timeframe)
