...
Every batch directory includes `summary.json`, `results.json`, and `scoreboard.json`. `summary.json.run_result` identifies the top successful run from the existing scoreboard order, other top candidates, and failed children without duplicating artifact paths or promotion commands. Batch summaries are discoverable with `quanttradeai runs list --type batch --json`.
...
The sweep batch also writes `summary.json`, `results.json`, and `scoreboard.json`. `summary.json.run_result` records the winning child and ranked candidates; child command details remain in `results.json`.
...
Batch scoreboards for paper mode sort by `total_pnl`. `summary.json.run_result` records the top successful paper run without recommending a promotion command.
...
Batch scoreboards for live mode sort by `total_pnl`. `summary.json.run_result` records the winning live run and failed children; metrics, executions, decisions, and logs stay in their normal run artifacts.
