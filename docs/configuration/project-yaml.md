...
Every batch directory includes `summary.json`, `results.json`, and `scoreboard.json`. `summary.json.run_result` identifies the top successful run from the existing scoreboard order, failed children, useful logs, important artifacts, and exact next commands for promotion or inspection. Batch summaries are discoverable with `quanttradeai runs list --type batch --json`.
...
The sweep batch also writes `summary.json`, `results.json`, and `scoreboard.json`. `summary.json.run_result` uses the winning child's existing `promote_command` and includes the follow-up paper command for the base agent.
...
Batch scoreboards for paper mode sort by `total_pnl`. `summary.json.run_result` recommends the live-promotion command for the top successful paper run.
...
Batch scoreboards for live mode sort by `total_pnl`. `summary.json.run_result` points to metrics, executions, decisions, and logs for inspection instead of producing a promotion command.
